    ...
    <div className="rounded-lg border border-border bg-card px-3 py-2 shadow-lg">
      ...
      {payload.map((entry) => (
        <div key={entry.dataKey} className="flex items-center gap-2 text-xs">
          ...
          <span className="font-mono tabular-nums ml-auto">{formatNumber(entry.value as number)}</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="h-52">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            />
            <Tooltip content={<ChartTooltip />} />
            <Bar dataKey="likes" stackId="engagement" fill="hsl(350 89% 60%)" radius={[0, 0, 0, 0]} />
            <Bar dataKey="replies" stackId="engagement" fill="hsl(263 83% 65%)" radius={[0, 0, 0, 0]} />
            <Bar dataKey="reposts" stackId="engagement" fill="hsl(160 84% 45%)" radius={[2, 2, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div className="w-full">
      ...
      {timeSeries && timeSeries.length > 1 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-10" data-testid="metrics-charts">
