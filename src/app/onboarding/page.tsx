...
function OnboardingContent() {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* Step content card */}
      <div className="w-full rounded-2xl border border-white/[0.06] bg-white/[0.03] p-8 backdrop-blur-md">
        {currentStep === "welcome" && (
...
function WelcomeStep({ onContinue }: { onContinue: () => void }) {
  return (
    <div className="flex flex-col items-center text-center">
      {/* Logo */}
      <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-xl border border-white/10 bg-white/5">
        <svg
          width="24"
          height="24"
          viewBox="0 0 20 20"
      ...
      <Button
        onClick={onContinue}
        className="w-full cursor-pointer bg-white py-6 text-sm font-semibold text-black transition-all duration-200 hover:bg-white/90"
      >
        Get Started
      </Button>
    </div>
    ...
    <div>
        ...
        <div className="flex items-center justify-between">
          ...
          {isConnected ? (
            ...
            <a
              ...
              className="inline-flex h-9 items-center rounded-lg border border-white/10 bg-white/5 px-4 text-xs font-medium text-white transition-colors hover:bg-white/10"
            >
              Connect
            </a>
          )}
      ...
      <div className="mt-3 space-y-2">
        {["LinkedIn", "Threads"].map((name) => (
          <div
            key={name}
            className="flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.01] p-4 opacity-40"
          >
            <div className="flex items-center gap-3">
      ...
      {isConnected && (
        <p className="mt-4 text-center text-xs text-white/30 animate-pulse">
          Continuing...
        </p>
      )}
    </div>
    ...
    <div>
      ...
      <div className="mb-5">
        ...
        <select
          ...
          className={cn(
            "w-full rounded-lg border bg-white/[0.03] px-3 py-2.5 text-sm text-white outline-none transition-colors",
            errors.niche
              ? "border-red-500/50"
        ...
        >
          ...
          {NICHES.map((niche) => (
            <option key={niche.value} value={niche.value} className="bg-[#1a1a1a]">
        ...
        {data.primary_niche === "other" && (
          <input
            ...
            className="mt-2 w-full rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2.5 text-sm text-white placeholder:text-white/20 outline-none focus:border-white/25"
            maxLength={100}
          />
        )}
        {errors.niche && (
      ...
      <div className="mb-6">
        ...
        <input
          ...
          className={cn(
            "w-full rounded-lg border bg-white/[0.03] px-3 py-2.5 text-sm text-white placeholder:text-white/20 outline-none transition-colors",
            errors.audience
              ? "border-red-500/50"
              : "border-white/10 focus:border-white/25",
          )}
        ...
        <div className="mt-1 flex items-center justify-between">
          {errors.audience ? (
            ...
            <p className="text-xs text-white/20">Be specific — who is your ideal reader?</p>
          )}
      ...
      <Button
        ...
        className="w-full cursor-pointer bg-white py-6 text-sm font-semibold text-black transition-all duration-200 hover:bg-white/90 disabled:opacity-50"
      >
  ...
  if (didImport) {
    return (
      <div className="flex flex-col items-center text-center">
        ...
        <Button
          onClick={onContinue}
          className="w-full cursor-pointer bg-white py-6 text-sm font-semibold text-black hover:bg-white/90"
        >
          Continue
        </Button>
      </div>
  ...
  if (isImporting) {
    return (
      <div className="flex flex-col items-center text-center py-4">
        <div className="mb-4 h-8 w-8 animate-spin rounded-full border-2 border-white/10 border-t-white/60" />
        <p className="text-sm text-white/60">Importing your posts...</p>
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {[
          ...
          <button
            ...
            className="flex w-full cursor-pointer items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 text-left transition-all hover:border-white/15 hover:bg-white/[0.04]"
          >
            <div>
              <p className="text-sm font-medium text-white">{option.label}</p>
              <p className="text-xs text-white/25">{option.time}</p>
            </div>
            <svg
              width="16"
              height="16"
      ...
      <button
        type="button"
        onClick={onSkip}
        className="mt-4 w-full cursor-pointer text-center text-xs text-white/30 transition-colors hover:text-white/50"
      >
        Skip for now
      </button>
    </div>
  ...
  return (
    <div>
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.03] p-6">
        ...
        <p className="text-sm text-white/40">{current.description}</p>
          ...
          {stops.map((_, i) => (
            <button
              ...
              className={cn(
                ...
                i === tourIndex ? "w-5 bg-white/60" : "w-1 bg-white/15",
              )}
            />
          ))}
        </div>
      ...
      <div className="mt-4 flex gap-2">
        {tourIndex > 0 && (
          <Button
            ...
            className="flex-1 cursor-pointer border-white/10 bg-white/5 text-sm text-white hover:bg-white/10"
          >
            Back
          </Button>
        )}
        {tourIndex < stops.length - 1 ? (
          <Button
            onClick={() => setTourIndex(tourIndex + 1)}
            className="flex-1 cursor-pointer bg-white py-5 text-sm font-semibold text-black hover:bg-white/90"
          >
            Next
          </Button>
        ) : (
      ...
      <button
        type="button"
        onClick={onContinue}
        className="mt-3 w-full cursor-pointer text-center text-xs text-white/25 transition-colors hover:text-white/40"
      >
        Skip tour
      </button>
    </div>
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <div className="w-full space-y-2">
          ...
          <Button
            ...
            className="w-full cursor-pointer border-white/10 bg-white/5 py-6 text-sm text-white hover:bg-white/10"
          >
            Create New Post
          </Button>
        </div>
        ...
        <div className="mt-6 w-full rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 text-left">
          ...
          <div className="space-y-1.5 text-xs text-white/30">
            <p>
              <span className="text-white/50">Post</span> — Create and publish
              content
            </p>
            <p>
              <span className="text-white/50">Observe</span> — We track
              performance automatically
            </p>
            <p>
              <span className="text-white/50">Learn</span> — Insights unlock
              after 20+ posts
            </p>
          </div>
    ...
    <div>
      ...
      {isLoadingIdeas && (
        <div className="mb-6 flex flex-col items-center py-4">
          <div className="mb-2 h-6 w-6 animate-spin rounded-full border-2 border-white/10 border-t-white/60" />
